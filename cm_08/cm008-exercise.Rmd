...
```{r}
gapminder %>% mutate(lifeExp = if_else(country=="Canada" & year==1952, 70, lifeExp)) %>%
...
```{r}
gapminder %>% summarise(mu = mean(lifeExp),
                        med = median(lifeExp))
```
...
gapminder %>% 
  mutate(lifeExpGroup = if_else(lifeExp > 60, "High", "Low")) %>% 
  group_by(continent, lifeExpGroup) %>% 
  summarize(md = median(gdpPercap)) %>% 
  arrange(md)
...
gapminder %>% 
  group_by(continent, country) %>% 
  mutate(FiveDiff = lifeExp - lag(lifeExp)) %>% 
  summarize(bigDrop = min(FiveDiff, na.rm=TRUE)) %>% 
  summarize(min = min(bigDrop))
```
